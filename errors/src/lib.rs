...
impl fmt::Display for MarshalingError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      Self::ArrayLengthExceeded => write!(f,
          "length of array is larger than the type allows"
      ),
      Self::UnexpectedEndOfBuffer => write!(f,
          "expected to have more buffer data but found none"
      ),
      Self::UnknownSelector => write!(f,
          "selector targeted is not known to the unmarshaling code"
      ),
      Self::MarshalingDeriveError => write!(f,
          "unexpected derive error"
      ),
    }
  }
}
